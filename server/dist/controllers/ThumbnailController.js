...
const buildPrompt = ({ title, style, aspectRatio, colorScheme, userPrompt, textOverlay, }) => {
    let prompt = `Create a ${stylePrompts[style]} thumbnail for: "${title}".`;
    if (colorScheme) {
...
const getPollinationsModel = () => process.env.POLLINATIONS_MODEL?.trim() || "gptimage";
const createPollinationsUrl = ({ prompt, width, height, seed, model, enhance, nologo, }) => {
...
const fetchPollinationsImage = async (imageUrl) => {
    const apiKey = getPollinationsApiKey();
    ...
    if (!response.ok) {
        ...
        throw new Error(`Free image provider failed with status ${response.status}${errorText ? `: ${errorText}` : ""}`);
    }
    const arrayBuffer = await response.arrayBuffer();
    return Buffer.from(arrayBuffer);
};
const createPollinationsSeed = () => Math.floor(Math.random() * 2147483647);
const generateWithPollinations = async ({ prompt, aspectRatio, }) => {
    if (!getPollinationsApiKey()) {
        throw new Error("Pollinations API key is missing. Set POLLINATIONS_API_KEY in server/.env.");
    }
    ...
    for (const imageUrl of requestUrls) {
        try {
            ...
            lastError = error;
            attemptErrors.push(`${imageUrl} -> ${lastError.message}`);
        }
    }
    console.warn("Pollinations request attempts failed:", attemptErrors);
    throw (lastError ??
...
export const generateThumbnail = async (req, res) => {
    ...
    try {
        ...
        const safeAspectRatio = aspect_ratio && aspect_ratio in aspectRatioDimensions
            ? aspect_ratio
            : "16:9";
